...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            out, rq_loss, indices, cl_loss = self.model(data)
            loss, loss_recon, quant_loss, cl_loss = self.model.compute_loss(
                out, rq_loss, cl_loss, xs=data)
            ...
            total_cl_loss += cl_loss.item() if isinstance(
                cl_loss, torch.Tensor) else cl_loss

        return total_loss, total_recon_loss, total_quant_loss, total_cl_loss
    ...
    def _valid_epoch(self, valid_data):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            src_indices, rec_indices = indices
            src_indices = src_indices.view(
                -1, src_indices.shape[-1]).cpu().numpy()
            for index in src_indices:
                code = "-".join([str(int(_)) for _ in index])
                src_indices_set.add(code)

            rec_indices = rec_indices.view(
                -1, rec_indices.shape[-1]).cpu().numpy()
            for index in rec_indices:
                code = "-".join([str(int(_)) for _ in index])
                rec_indices_set.add(code)

        src_collision_rate = (num_sample -
                              len(list(src_indices_set))) / num_sample
        rec_collision_rate = (num_sample -
                              len(list(rec_indices_set))) / num_sample
        collision_rate = (src_collision_rate + rec_collision_rate) / 2.0
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, loss,
        ...
        train_loss_output += "cl loss" + ": %.4f" % cl_loss
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            train_loss, train_recon_loss, train_quant_loss, train_cl_loss = self._train_epoch(
                data, epoch_idx)
            training_end_time = time()
            train_loss_output = self._generate_train_loss_output(
                epoch_idx, training_start_time, training_end_time, train_loss,
                train_recon_loss, train_quant_loss, train_cl_loss)
